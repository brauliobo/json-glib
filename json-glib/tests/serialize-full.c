...
test_deserialize (void)
{
  ...
  error = NULL;
  object = json_gobject_from_data (FLOW_TYPE, flow_test, -1, &error);
  if (error)
    g_error ("*** Unable to parse buffer: %s\n", error->message);

  flow = FLOW (object);
  g_assert (DICTIONARY_IS_OBJECT (flow->dictionary));
  dictionary = DICTIONARY (flow->dictionary);
